...
class ProdutoController {
  ...
  async index(req, res, next) {
    try {
      ...
      const produtos = await Produto.paginate({ loja: loja }, { offset: offset, limit: limit, sort: getSort(req.query.sortType) });
  ...
  async search(req, res, next) {
    try {
      ...
      const produtos = await Produto.paginate(
        {
          loja: loja,
          $or: [{ titulo: { $regex: search } }, { descricao: { $regex: search } }, { sku: { $regex: search } }],
        },
        { offset: offset, limit: limit, sort: getSort(req.query.sortType) }
      );
      return res.send({ produtos });
